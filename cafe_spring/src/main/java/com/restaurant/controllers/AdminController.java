...
public class AdminController {
    ...
    public ResponseEntity<?> updateProduct(@PathVariable Long productId, @ModelAttribute ProductDto productDto) throws IOException {
        ProductDto upadtedProductDto = adminService.updateProduct(productId, productDto);
        if (upadtedProductDto == null)
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Something went wrong");
        return ResponseEntity.status(HttpStatus.OK).body(upadtedProductDto);
    }
